...
指数分布の一般化 ($Exp(\lambda) = Ga(1, \lambda)$)。
- 表記: $Ga(\alpha, \lambda)$ ($\alpha$: 形状母数, $\lambda$: 尺度母数/レートパラメータ)
- PDF: $f(x) = \frac{\lambda^\alpha}{\Gamma(\alpha)} x^{\alpha-1} e^{-\lambda x} \quad (x > 0)$

$$
E[X] = \frac{\alpha}{\lambda}, \quad V[X] = \frac{\alpha}{\lambda^2}, \quad M_X(t) = \left( 1 - \frac{t}{\lambda} \right)^{-\alpha}
$$
...
> [!WARNING]
> **【パラメータ定義の注意 (公式準拠)】**
> 本ノートでは、統計検定公式テキストに準拠して **レートパラメータ定義 ($ \lambda $)** を採用します。
> $$ f(x) \propto e^{-\lambda x} $$
>
> 多くの機械学習ライブラリ（scipy.stats.gammaなど）や一部の教科書では、逆数であるスケールパラメータ $\theta = 1/\lambda$ を用いる場合があるため、実装や参照の際は注意してください。
...
> つまり **$X_{(1)} \sim Exp(n\lambda)$** となります（パラメータが $n$ 倍になる）。
...
| **ガンマ分布** | **カイ二乗分布** | $\chi^2(n) = Ga(n/2, 1/2)$ |
| **ガンマ分布** | **指数分布** | $Exp(\lambda) = Ga(1, \lambda)$ |
...
| **ガンマ分布** | $Ga(a, b)$ | $\propto x^{a-1}e^{-x/b}$ | $ab$ | $ab^2$ | $Exp(\lambda)$の一般化 |
