...
指数分布の一般化 ($Exp(1/b) = Ga(1, b)$)。
- 表記: $Ga(a, b)$ ($a$: 形状母数, $b$: 尺度母数)
- PDF: $f(x) = \frac{1}{\Gamma(a)b^a} x^{a-1} e^{-x/b} \quad (x > 0)$

$$
E[X] = ab, \quad V[X] = ab^2, \quad M_X(t) = (1-bt)^{-a}
$$

![ガンマ分布](assets/images/gamma.png)

> [!WARNING]
> **【パラメータ定義の注意 (スケール vs レート)】**
> ガンマ分布 $Ga(a, b)$ の第2パラメータ $b$ には、文献によって2通りの定義があります。
> 1. **スケールパラメータ定義**（本ノートで採用）: $\exp(-x/b)$ の形。平均は $ab$。
> 2. **レートパラメータ定義**（統計検定公式などで見られる）: $\exp(-\lambda x)$ の形（$b=1/\lambda$）。平均は $a/\lambda$。
>
> この違いにより、カイ二乗分布 $\chi^2(n)$ との対応も変わります。
> - スケール定義 ($b$): $Ga(n/2, 2)$
> - レート定義 ($\lambda$): $Ga(n/2, 1/2)$
...
| **ガンマ分布** | **カイ二乗分布** | $\chi^2(n) = Ga(n/2, 2)$ ※スケール定義 |
| **ガンマ分布** | **指数分布** | $Exp(\lambda) = Ga(1, 1/\lambda)$ ※スケール定義 |
