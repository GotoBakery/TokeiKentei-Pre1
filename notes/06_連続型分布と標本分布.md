...
> [!TIP]
> **【準1級合格のポイント：一様分布の分散】**
> $V[X] = \frac{(b-a)^2}{12}$ という公式は忘れがちです。「幅の2乗を12で割る」と語呂で覚えましょう。
> 積分計算で導出する時間は試験中にはありません。
...
> [!TIP]
> **【準1級合格のポイント：カイ二乗分布の構成】**
> 「標準正規分布の2乗和はカイ二乗分布に従う」 ($\sum Z_i^2 \sim \chi^2(n)$)。
> この事実は、適合度検定や分散の検定など、あらゆる場面で検定統計量を導出する基礎となります。
...
> [!TIP]
> **【準1級合格のポイント：多変量正規分布の条件付き期待値】**
> 多変量正規分布において、ある変数を固定したときの条件付き平均 $E[Y|X=x]$ は、$x$ の**線形関数（回帰直線）**になります。
> 「正規分布の条件付き分布もまた正規分布」という性質は重要です。
...
> [!TIP]
> **【準1級合格のポイント：最大値・最小値のPDF】**
> 最大値 $X_{(n)}$ の分布関数 $F_{max}(x) = \{F(x)\}^n$ は「全員が $x$ 以下」と考えれば自明です。
> これを微分して $f_{max}(x) = n\{F(x)\}^{n-1}f(x)$ を導く手順をマスターしましょう。丸暗記より導出の方が確実です。
...
> [!TIP]
> **【準1級合格のポイント：関係図のイメージ】**
> 「正規分布」を中心に、二乗して和をとると「カイ二乗」、比をとると「t」や「F」になるといった系譜を頭の中で描けるようにしましょう。
> 検定手法を選択する際、このマップが頭にあると迷いません。
...
```mermaid
graph TD
    Normal[正規分布 N(μ, σ²)] -->|標準化| StdNormal[標準正規分布 N(0, 1)]
    StdNormal -->|二乗和| ChiSq[カイ二乗分布 χ²]
    ChiSq -->|比| F[F分布]
    StdNormal -->|比| t[t分布]
    ChiSq -->|分母| t
    Gamma[ガンマ分布] -->|一般化| ChiSq
    Gamma -->|一般化| Exp[指数分布]
    
    style Normal fill:#f9f,stroke:#333
    style StdNormal fill:#ccf,stroke:#333
```
...
> [!TIP]
> **【準1級合格のポイント：暗記すべき分散】**
> 一様分布 $(b-a)^2/12$ と 指数分布 $1/\lambda^2$ は特に忘れやすいので要注意です。
> 正規分布の $\sigma^2$ は定義そのものです。
