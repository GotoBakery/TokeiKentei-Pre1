...
> [!TIP]
> **【準1級合格のポイント：収束の強弱関係】**
> 「概収束 $\Rightarrow$ 確率収束 $\Rightarrow$ 分布収束」という包含関係（強い方ならば弱い方も成り立つ）は頻出です。
> 逆は成り立ちません（分布収束しても確率収束するとは限らない）。
...
```mermaid
graph TD
    AS[概収束 (a.s.)] -->|implies| Prob[確率収束 (p)]
    Mean[平均収束 (Lp)] -->|implies| Prob
    Prob -->|implies| Dist[分布収束 (d)]
...
> [!TIP]
> **【準1級合格のポイント：CLTの適用条件】**
> 中心極限定理 (CLT) が使えるのは「サンプルサイズ $n$ が十分大きいとき」だけでなく、「分散が有限である」ことも条件です。
> コーシー分布のように分散が発散する場合、平均をとっても正規分布にはなりません（安定分布）。
...
> [!TIP]
> **【準1級合格のポイント：スルツキーの補題の実用性】**
> 「確率収束する変数（分散 $S^2$ など）」と「分布収束する変数（正規化平均など）」を掛け算したとき、前者をただの**定数**とみなして扱って良い、というのがこの定理の御利益です。
> $t$分布の導出（$S \to \sigma$）で暗黙のうちに使っています。
...
> [!TIP]
> **【準1級合格のポイント：t統計量の収束】**
> $T = \frac{\bar{X}-\mu}{S/\sqrt{n}}$ が $n \to \infty$ で $Z \sim N(0,1)$ に近づく理由は、スルツキーの補題で説明できます。
> 1. 分子: $\sqrt{n}(\bar{X}-\mu) \xrightarrow{d} Z$ (CLT)
> 2. 分母: $S \xrightarrow{p} \sigma$ (一致性) なので $1/S \xrightarrow{p} 1/\sigma$ (連続写像定理)
> 3. 積: $\frac{\sqrt{n}(\bar{X}-\mu)}{S} = \sqrt{n}(\bar{X}-\mu) \cdot \frac{1}{S} \xrightarrow{d} Z \cdot \frac{1}{\sigma} \times \sigma = Z$ (スルツキー)
> これにより $t(n) \xrightarrow{d} N(0,1)$ が示されます。
